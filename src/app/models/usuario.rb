...
class Usuario < ApplicationRecord
  ...
  validates :password,
            presence: { message: "não pode ser vazia" },
            length: { minimum: 6, message: "precisa ter no mínimo 6 caracteres" },
            confirmation: { message: "não confere com a confirmação" }

  validates :password_confirmation,
            presence: { message: "a confirmação de senha precisa ser preenchida" }

  # valida a senha atual no update
  validates :current_password,
            presence: { message: "precisa ser preenchida" },
            on: :update
